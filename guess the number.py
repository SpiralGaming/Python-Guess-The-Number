...
while True:
    command = Write.Input("Please enter a command: ", Colors.dark_red, interval=0.0025, hide_cursor=False)
    if command in ("help", "h", "10", "ten", "100", "hundred", "custom", "c", "exit", "e", "quit", "q"):
        ...
        if command == "10" or command == "ten":
            ...
            guess = int(Write.Input("\nGuess a number between 1 and 10: ", Colors.dark_red, interval=0.0025, hide_cursor=False))
            if guess == number:
        ...
        if command == "100" or command == "hundred":
            ...
            guess = int(Write.Input("\nGuess a number between 1 and 100\n", Colors.dark_red, interval=0.0025, hide_cursor=False))
            if guess == number:
                Write.Print(f"\nYou guessed correctly!\n The number is {number}\n", Colors.green, interval=0.0025)
            else:
                Write.Print(f"\nYou guessed incorrectly!\n The number is {number}\n", Colors.red, interval=0.0025)
        if command == "custom" or command == "c":
            number = random.randint(1, int(Write.Input("Please enter a number: ", Colors.dark_red, interval=0.0025, hide_cursor=False)))
            guess = int(Write.Input("Please guess the number: ", Colors.dark_red, interval=0.0025, hide_cursor=False))
            if guess == number:
